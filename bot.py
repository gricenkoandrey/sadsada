...
def admin_main_kb():
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton('📊 Статистика', callback_data='adm_stats'), InlineKeyboardButton('👤 Пользователи', callback_data='adm_users')],
        [InlineKeyboardButton('💳 Заявки', callback_data='adm_orders'), InlineKeyboardButton('📝 Логи', callback_data='adm_logs')],
        [InlineKeyboardButton('⭐ Управлять Premium', callback_data='adm_manage'), InlineKeyboardButton('⬅️ Назад', callback_data='adm_back')],
    ])
    return kb
...
async def buy_cb(c: types.CallbackQuery):
    ...
    lang = get_user(c.from_user.id).get('lang', DEFAULT_LANG)
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(f"Перевести на карту: {CARD_NUMBER}", callback_data='copy_card')],
...
async def adm_cb(c: types.CallbackQuery):
    ...
    if cmd == 'adm_stats':
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton('Выдать Premium (ID)', callback_data='adm_grant_prompt')],
            [InlineKeyboardButton('Забрать Premium (ID)', callback_data='adm_revoke_prompt')],
